...
class InputSearch extends Component {
	...
	onFocus() {
		...
		if (search && !this.loading && !(suggestions && suggestions.length)) {
	...
	onKeyDown(event) {
		...
		if (!showSuggestions || !suggestions.length || loading) {
			...
			switch (event.keyCode) {
				...
				// last position.
				case DOWN: {
					if (this.state.search.length !== event.target.selectionStart) {
						event.stopPropagation();
						event.preventDefault();

						// Set the input caret to the last position
						event.target.setSelectionRange(this.state.search.length, this.state.search.length);
					}
					break;
				}
			}
	...
	renderSettings() {
		const { isSettingsExpanded, additionalControls, advancedOptions } = this.props;
		return (
			<Fragment>
...
export default compose(withSafeTimeout, withInstanceId)(InputSearch);
