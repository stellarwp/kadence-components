							...
							{colors && (
								<div className="kadence-pop-color-palette-swatches">
									{map(colors, ({ color, slug, name }) => {
										...
										const isActive =
											(slug.startsWith('theme-palette') && color === point.color) ||
											(slug.startsWith('theme-palette') && pointColor === color);
										const isLinked =
											!slug.startsWith('theme-palette') &&
													...
													<Button
														...
														onClick={() => {
															if (
																...
																onChange(
																	updateControlPointColor(controlPoints, index, color)
																);
															} else {
																onChange(
																	updateControlPointColor(
						...
						<ColorPicker
							...
							onChange={(color) => {
								...
								if (!alreadyInsertedPoint) {
									onChange(
										addControlPoint(controlPoints, insertPosition, colord(color.rgb).toRgbString())
									);
					...
					{colors && (
						<div className="kadence-pop-color-palette-swatches">
							{map(colors, ({ color, slug, name }) => {
								...
								const isActive =
									(slug.startsWith('theme-palette') && color === tempColor) ||
									(slug.startsWith('theme-palette') && pointColor === color);
								const isLinked =
									!slug.startsWith('theme-palette') &&
									color.startsWith('var(') &&
								...
								return (
									<div key={color} className="kadence-color-palette__item-wrapper">
											...
											<Button
												...
												onClick={() => {
													...
													if (slug.startsWith('theme-palette') && !color.startsWith('var(')) {
														...
														if (!alreadyInsertedPoint) {
															onChange(
																addControlPoint(controlPoints, insertPosition, color)
															);
															setAlreadyInsertedPoint(true);
														} else {
															onChange(
										...
										{isActive && <Dashicon icon="admin-site" />}
										{!slug.startsWith('theme-palette') && tempColor === color && (
											<Dashicon icon="saved" />
										)}
									</div>
								);
