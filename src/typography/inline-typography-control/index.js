...
import { Button, ButtonGroup, Dashicon, Tooltip, Dropdown, ToggleControl, SelectControl } from '@wordpress/components';
...
class InlineTypographyControls extends Component {
	...
	componentDidMount() {
		const fontsarray =
			typeof getBlocksParams() !== 'undefined' && getBlocksParam('g_font_names')
		...
		if (typeof getBlocksParams() !== 'undefined' && getBlocksParam('c_fonts')) {
			const newOptions = [];
		...
		if (typeof getBlocksParams() !== 'undefined' && getBlocksParam('isKadenceT')) {
			const themeOptions = [
				{
		...
		let typographySelectOptions = [].concat.apply(
			[],
		...
		if (
			...
			if (
				blockConfigObject['kadence/typography'].showAll !== undefined &&
				!blockConfigObject['kadence/typography'].showAll
			) {
	...
	setTypographyOptions(typographySelectOptions) {
		...
		const isKadenceT = typeof getBlocksParams() !== 'undefined' && getBlocksParam('isKadenceT') ? true : false;
		const headingWeights =
			typeof getBlocksParams() !== 'undefined' && getBlocksParam('headingWeights')
				? getBlocksParam('headingWeights')
				: [];
		const bodyWeights =
			typeof getBlocksParams() !== 'undefined' && getBlocksParam('bodyWeights')
				? getBlocksParam('bodyWeights')
				: [];
		const buttonWeights =
			typeof getBlocksParams() !== 'undefined' && getBlocksParam('buttonWeights')
				? getBlocksParam('buttonWeights')
				: [];
		if (
			isKadenceT &&
			this.props.fontGroup === 'heading' &&
			headingWeights &&
			Array.isArray(headingWeights) &&
			headingWeights.length
		) {
			standardWeights = headingWeights;
		}
		if (
			isKadenceT &&
			this.props.fontGroup === 'body' &&
			bodyWeights &&
			Array.isArray(bodyWeights) &&
			bodyWeights.length
		) {
		...
		if (
			isKadenceT &&
			this.props.fontGroup === 'button' &&
			buttonWeights &&
			Array.isArray(buttonWeights) &&
			buttonWeights.length
		) {
		...
		if (
			...
			this.props.fontFamily &&
			typeof getBlocksParams() !== 'undefined' &&
			...
			if (selected === null) {
				...
				if (selected.google) {
					if (
						typeof getBlocksParams() !== 'undefined' &&
						getBlocksParam('g_fonts') &&
						!getBlocksParam('g_fonts')[selected.value].v.includes('regular')
					) {
						variant = getBlocksParam('g_fonts')[selected.value].v[0];
					} else {
						variant = 'regular';
					}
					if (
						typeof getBlocksParams() !== 'undefined' &&
						getBlocksParam('g_fonts') &&
						!getBlocksParam('g_fonts')[selected.value].w.includes('regular')
					) {
						weight = getBlocksParam('g_fonts')[selected.value].w[0];
					} else {
						weight = '400';
					}
					if (
						typeof getBlocksParams() !== 'undefined' &&
						getBlocksParam('g_fonts') &&
						getBlocksParam('g_fonts')[selected.value].s.length > 1
					) {
						subset = 'latin';
					} else {
						subset = '';
										...
										<div className="components-base-control kt-typography-number-input">
											<div className="components-base-control__field">
												<label
													...
													htmlFor={`kt-inline-spacing${uniqueID}`}
												>
													{__('Spacing')}
												</label>
												<input
