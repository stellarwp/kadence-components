...
export default function ResponsiveBorderControl({
	...
	useEffect(() => {
		...
		if (isEqual(value?.[0]?.unit, mobileValue?.[0]?.unit) && mobileValue) {
			mobileValue[0].unit = '';
			onChangeMobile(mobileValue);
		}
		if (isEqual(value?.[0]?.unit, tabletValue?.[0]?.unit) && tabletValue) {
			tabletValue[0].unit = '';
	...
	return [
		onChange && onChangeTablet && onChangeMobile && (
			<div
				ref={ref}
				className={'components-base-control kb-responsive-border-control kadence-border-box-control'}
			>
