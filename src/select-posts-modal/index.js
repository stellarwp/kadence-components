...
import { KadenceSelectTerms } from '@kadence/components';

import { Button, Modal, Flex, FlexItem, FlexBlock, Spinner, TextControl, Dashicon } from '@wordpress/components';
...
function SelectPostsModal(props) {
	...
	const showCategoryFilter =
		showTagFilter || categoryRestBase === 'kt-wc-blocks/v1/products/categories' ? true : false;
		...
		return (
			<tr key={post.id} className={tmpSelectedPosts.includes(post.id) ? 'is-selected' : ''}>
				...
				>
					...
					<strong>Type: </strong> {post.type} <strong>Published: </strong>{' '}
	...
	return (
		<>
			<Button isLarge={true} isSecondary={true} onClick={(e) => setIsOpen(true)}>
				{buttonLabel}
			</Button>
