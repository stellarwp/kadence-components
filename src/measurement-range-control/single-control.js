...
export default function SingleMeasureRangeControl({
	...
	return [
		onChange && (
			<div
				className={`components-base-control component-spacing-sizes-control kadence-single-measure-range-control${
					className ? ' ' + className : ''
				}`}
			>
				{!setCustomControl && label && (
					<Flex justify="space-between" className={'kadence-radio-range__header'}>
						<FlexItem>
						...
						{isPopover && (
							<>
								<Button
									...
									onMouseOut={onMouseOut}
								>
									{parentLabel && label && <span className="kadence-placement-label">{label}</span>}
									<span className="kadence-spacing-btn-val">
										{reviewOptions[currentValue]?.label}
										{!reviewOptions[currentValue]?.label && (
											<span className="kadence-spacing-btn-placeholder">
												{reviewOptions?.[currentPlaceholder]?.label
													? reviewOptions?.[currentPlaceholder]?.label
													: placeholder}
											</span>
										)}
								...
								{isOpen && (
									<Popover onClose={close} className={'kadence-range-popover-settings'}>
