...
export default function KadenceWebfontLoader({ typography, clientId, id, children }) {
	const [status, setStatus] = useState(undefined);
	const { addWebFont } = useDispatch('kadenceblocks/data');
	const [device, setDevice] = useState('desktop');
	const { previewDevice, isUniqueFont } = useSelect((select) => {
		return {
	...
	const loadFonts = () => {
		setTimeout(() => {
			if (
				undefined !== nConfig?.google?.families?.[0] &&
				isUniqueFont(nConfig?.google?.families?.[0], frames['editor-canvas'] ? 'Desktop' : 'iframe')
			) {
				WebFont.load({
					...nConfig,
