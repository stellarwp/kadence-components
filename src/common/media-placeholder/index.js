...
import { Button, FormFileUpload, Placeholder, DropZone } from '@wordpress/components';
	...
	<URLPopover onClose={onClose}>
		<form className="block-editor-media-placeholder__url-input-form" onSubmit={onSubmit}>
			<input
				...
				type="url"
				aria-label={__('URL', '__KADENCE__TEXT__DOMAIN__')}
			...
			<Button
				...
				icon={keyboardReturn}
				label={__('Apply', '__KADENCE__TEXT__DOMAIN__')}
				type="submit"
			/>
		</form>
	</URLPopover>
...
class KadenceMediaPlaceholder extends Component {
		...
		if (multiple) {
			if (addToGallery) {
				...
				setMedia = (newMedia) => {
					...
					const filteredMedia = (this.props.value || []).filter((item) => {
	...
	renderPlaceholder(content, onClick) {
		...
		if (!mediaUpload && !onSelectURL) {
			instructions = __('To edit this block, you need permission to upload media.', '__KADENCE__TEXT__DOMAIN__');
		}
		...
		if (instructions === undefined || title === undefined) {
			...
			if (instructions === undefined && mediaUpload) {
				instructions = __('Upload a media file or pick one from your media library.', '__KADENCE__TEXT__DOMAIN__');
			...
			//  if ( title === undefined ) {
			// 	 title = __( 'Media', '__KADENCE__TEXT__DOMAIN__' );

			// 	 if ( isAudio ) {
			// 		 title = __( 'Audio', '__KADENCE__TEXT__DOMAIN__' );
			// 	 } else if ( isImage ) {
			// 		 title = __( 'Image', '__KADENCE__TEXT__DOMAIN__' );
			// 	 } else if ( isVideo ) {
		...
		return (
			onCancel && (
				<Button
					...
					title={__('Cancel', '__KADENCE__TEXT__DOMAIN__')}
					isLink
					onClick={onCancel}
				>
		...
		return (
			<div className="block-editor-media-placeholder__url-input-container">
				<Button
					...
					isTertiary
				>
					{__('Insert from URL', '__KADENCE__TEXT__DOMAIN__')}
				</Button>
				{isURLInputVisible && (
