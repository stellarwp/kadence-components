 ...
 * Import Icons
 */
import { video } from '@kadence/icons';
...
class KadenceVideoControl extends Component {
	...
	render() {
		const { label, hasVideo, onSaveVideo, onRemoveVideo, disableMediaButtons, videoURL, videoID } = this.props;
		return (
						...
						<div className="components-kadence-video-btns">
							<MediaUpload
								...
								render={({ open }) => (
									<Button
										className={
											'components-button components-icon-button kb-cta-upload-btn kb-video-edit'
										}
										onClick={open}
										icon={video}
									>
