...
import { Button, Popover, TextControl, ButtonGroup } from '@wordpress/components';
...
export default function MeasurementSingleControl({
	...
	return (
		<div className={`measure-input-wrap measure-input-${placement}`}>
			<div className={'input-setting-toggle'}>
				<Button
					isSmall
					className={'input-setting-toggle-btn'}
					disabled={measurement && 'auto' == measurement ? true : false}
					tabIndex="-1"
					onClick={measurement && 'auto' == measurement ? '' : toggle}
				>
					{label}
				</Button>
				{isOpen && (
