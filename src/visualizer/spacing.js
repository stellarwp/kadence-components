...
export default function SpacingVisualizer({
	...
	const outputStyling = useMemo(() => {
		if (type === 'outside') {
			const spacingTop = outputSpacing?.top ? outputSpacing?.top : 0;
			const spacingRight = outputSpacing?.right ? outputSpacing?.right : 0;
			const spacingBottom = outputSpacing?.bottom ? outputSpacing?.bottom : 0;
			const spacingLeft = outputSpacing?.left ? outputSpacing?.left : 0;
			...
			return {
				borderTopWidth: outputSpacing?.top ? outputSpacing?.top : 0,
				borderRightWidth: outputSpacing?.right ? outputSpacing?.right : 0,
				borderBottomWidth: outputSpacing?.bottom ? outputSpacing?.bottom : 0,
				borderLeftWidth: outputSpacing?.left ? outputSpacing?.left : 0,
	...
	return (
		<div className={`kb__spacing-visualizer-wrap kb__spacing_visualizer-type-${type}`} style={style}>
