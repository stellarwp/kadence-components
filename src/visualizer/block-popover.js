...
function BlockPopover(
	...
	const popoverAnchor = useMemo(() => {
		...
		return {
			getBoundingClientRect() {
				...
				const left = Math.min(selectedBCR.left ?? Infinity);
				const top = Math.min(selectedBCR.top ?? Infinity);
				const right = Math.max(selectedBCR.right ?? -Infinity);
				const bottom = Math.max(selectedBCR.bottom ?? -Infinity);
	...
	}, [selectedElement, popoverDimensionsRecomputeCounter]);
	if (!selectedElement) {
		return null;
	}
	return (
		<Popover
			...
			shift={shift}
			{...props}
			className={classnames('kb-block-editor-block-popover', props.className)}
		>
